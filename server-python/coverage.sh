...
if [ "$1" = "html" ] ; then
  ...
  gcovr -r lib/nle --filter lib/nle/src --exclude lib/nle/src/nle.c --html-details -o coverage/htmlreport/report.html
else
  ...
  gcovr -r lib/nle --filter lib/nle/src --exclude lib/nle/src/nle.c --json-summary-pretty --json-summary -o coverage/"${filename}"
fi
