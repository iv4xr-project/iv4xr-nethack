...
/**
 * Provide a convenient reader/writer to read and write objects over a socket. This allows an object
 * to be sent over the socket (to a recipient on the other side of the socket-connection), encoded
 * as a Json-string. Similarly, the reader can receive an object, encoded as a Json-string, that was
 * sent to this class over the socket. Note that this implies that the object sent like this must be
...
public class ObjectReaderWriter_OverSocket {
  ...
  private static final Gson gson =
      new GsonBuilder()
          ...
          .serializeNulls()
          .registerTypeAdapter(Seed.class, new SeedTypeAdapter())
          .serializeNulls()
  ...
  /**
   * Send an object to the host. The object will first be serialized to a json string, so it is
   * assumed that the json serializer knows how to handle the object.
   *
   * @throws IOException
   */
  ...
  /**
   * Read an object that was sent by the host. The object will be received as a json string, which
   * is then converted into an instance of the given class. It is assumed that the json deserializer
   * knows how to do this. The resulting object is then returned.
   *
   * @throws
   */
