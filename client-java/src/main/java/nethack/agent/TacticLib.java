...
public class TacticLib implements IInteractiveWorldTacticLib<Pair<Integer,Tile>> {
	 ...
	 */
	public static List<Pair<Integer,Tile>> adjustedFindPath(AgentState state, 
			int level0,
			int x0, int y0, 
			int level1, int x1, int y1) {
	 ...
	 */
	public static List<WorldEntity> nearItems(AgentState S, EntityType itemType, int withinDistance) {
		//var player = S.worldmodel.elements.get(S.worldmodel.agentId) ;
	...
	Action navigateToAction(int levelId, int x, int y) {
		...
		.do2((AgentState S) ->  (Tile nextTile) -> {
	...
	Action navigateToAction(String targetId) {
		return action("move-to")
				...
				.on((AgentState S) -> {
					...
					var path = adjustedFindPath(S, Utils.levelId(a), agentPos.x, agentPos.y, Utils.levelId(e),target.x, target.y) ;
					if (path == null) {
						return null ;
					}
	...
	public Predicate<AgentState> hasHealPot_and_HpLow = S -> {
		var player = S.worldmodel.elements.get(S.worldmodel.agentId) ;
