...
public class NavAction {
    ...
    public static Action navigateTo(int levelNr, int x, int y) {
        return action("move-to").do2((AgentState S) -> (Tile nextTile) -> {
            logger.info(String.format(">>> navigateTo %d (%d, %d) via %s", levelNr, x, y, nextTile));
            WorldModel newwom = NavUtils.moveTo(S, nextTile);
            return new Pair<>(S, newwom);
        }).on((AgentState S) -> NavUtils.nextTile(NavUtils.adjustedFindPath(S, NavUtils.loc3(S.worldmodel.position), NavUtils.loc3(levelNr, x, y))));
    }
    ...
    public static Action navigateTo(String targetId) {
        return action("move-to").do2((AgentState S) -> (Tile nextTile) -> {
            ...
            return NavUtils.nextTile(NavUtils.adjustedFindPath(S, NavUtils.loc3(S.worldmodel.position), NavUtils.loc3(e.position)));
        });
    }
    ...
    public static Action explore(Pair<Integer, Tile> heuristicLocation) {
        return action("explore").do2((AgentState S) -> (Tile nextTile) -> {
            ...
            if (heuristicLocation == null) {
                ...
                return NavUtils.nextTile(S.multiLayerNav.explore(NavUtils.loc3(agentPos), heuristicLocation));
            }
        });
    }
}
