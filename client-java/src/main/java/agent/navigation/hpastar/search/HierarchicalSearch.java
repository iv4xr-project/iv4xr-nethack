...
{
    public List<AbstractPathNode> doHierarchicalSearch(HierarchicalMap map, Id<AbstractNode> startNodeId, Id<AbstractNode> targetNodeId, int maxSearchLevel, int maxPathsToRefine) {
    ...
    private List<AbstractPathNode> getPath(HierarchicalMap map, Id<AbstractNode> startNodeId, Id<AbstractNode> targetNodeId, int level, boolean mainSearch) {
    ...
    public List<AbstractPathNode> refineAbstractPath(HierarchicalMap map, List<AbstractPathNode> path, int level, int maxPathsToRefine) {
        ...
        {
            if (path.get(i).level == level && path.get(i).level == path.get(i - 1).level && map.belongToSameCluster(path.get(i).id, path.get(i - 1).id, level) && calculatedPaths < maxPathsToRefine)
            {
                List<AbstractPathNode> interNodePath = getPath(map, path.get(i - 1).id, path.get(i).id, level - 1, false);
    ...
    public List<IPathNode> abstractPathToLowLevelPath(HierarchicalMap map, List<AbstractPathNode> abstractPath, int mapWidth, int maxPathsToCalculate) {
        ...
        for (int currentPoint = 1;currentPoint < abstractPath.size();currentPoint++)
        {
            ...
            if (currentNodeClusterId == lastNodeClusterId && calculatedPaths < maxPathsToCalculate)
            {
                ...
                List<Id<ConcreteNode>> localPath = cluster.getPath(lastAbstractNodeId, currentAbstractNodeId);
                List<IPathNode> concretePath = new ArrayList<>();
                for (int i = 1;i < localPath.size();i++)
                {
                    int concreteNodeId = localNodeId2ConcreteNodeId(localPath.get(i).getIdValue(), cluster, mapWidth);
